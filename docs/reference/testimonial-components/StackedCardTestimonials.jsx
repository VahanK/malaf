...
// Fit: company-logo carousel, homepage-only. Same no-fake-proof caveat as the walls.
// Original used react-icons/si brand logos (Nike/Atlassian/etc.) — for WorkWith
// these would be real freelancer names/quotes, no company logos.
  ...
  return (
    <section className="bg-white py-24 px-4 lg:px-8 grid items-center grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-4 overflow-hidden">
      <div className="p-4">
        ...
        <p className="text-slate-500 my-4">Lorem ipsum dolor sit amet.</p>
      ...
      {Array.from(Array(numTracks).keys()).map((n) => (
        <button key={n} onClick={() => setSelected(n)} className="h-1.5 w-full bg-slate-300 relative">
          {selected === n ? (
            ...
            <span className="absolute top-0 left-0 bottom-0 bg-slate-950" style={{ width: selected > n ? "100%" : "0%" }} />
          )}
        </button>
      ))}
    </div>
  ...
  return (
    <div className="p-4 relative h-[450px] lg:h-[500px] shadow-xl">
      {testimonials.map((t, i) => (
        <Card {...t} key={i} position={i} selected={selected} setSelected={setSelected} />
      ))}
    </div>
  );
};

const Card = ({ Icon, description, name, title, position, selected, setSelected }) => {
  ...
  return (
    <motion.div
      initial={false}
      style={{ zIndex: position, transformOrigin: "left bottom", background, color }}
...
const testimonials = [
  { Icon: SiNike, description: "Lorem ipsum dolor sit amet.", name: "Jane Dodson", title: "Marketing Director, Nike" },
  { Icon: SiAtlassian, description: "Lorem ipsum dolor sit amet.", name: "Johnathan Rodriguez", title: "UX Research, Atlassian" },
  { Icon: SiDribbble, description: "Lorem ipsum dolor sit amet.", name: "Phil Heath", title: "Staff Engineer, Dribbble" },
  { Icon: SiGrubhub, description: "Lorem ipsum dolor sit amet.", name: "Andrea Beck", title: "Marketing Manager, GrubHub" },
  { Icon: SiKaggle, description: "Lorem ipsum dolor sit amet.", name: "Daniel Henderson", title: "Engineering Manager, Kaggle" },
  { Icon: SiSlack, description: "Lorem ipsum dolor sit amet.", name: "Anderson Lima", title: "Product Manager, Slack" },
];
